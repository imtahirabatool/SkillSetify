...
const ResumeTemplateA = ({ person = {}, lang = {}, contactLinks = {} }) => {
  ...
  const handleCapture = () => {
    if (resumeRef.current) {
      html2canvas(resumeRef.current, {
        ...
        scrollX: 0, // Adjust scroll position if needed
        scrollY: -window.scrollY, // Adjust scroll position if needed
        backgroundColor: null, // Preserve the background color
      }).then((canvas) => {
        const imgData = canvas.toDataURL('image/png');
        const link = document.createElement('a');
        link.href = imgData;
        link.download = 'resume_template.png'; // This will download the image
        link.click();
      }).catch((error) => {
  ...
  return (
    <div className="resume-container">
      <div id="resume3" ref={resumeRef} className="resume font-sans pb-12">
          ...
          <div className="absolute z-20 right-[15%] top-[50px] flex">
            <div className="overflow-hidden relative">
              <div
                ...
                style={{ backgroundImage: `url('/images/template1.png')` }}
              ></div>
        ...
        <div className="resume-content mt-[435px] ml-[15%] w-[70%]">
          ...
          <div className="skill-section">
            ...
            {person.skills?.map((skill) => (
              <div className="skill-block flex items-center mb-2" key={skill.name}>
          ...
          <div className="contact mt-0">
            ...
            <a href={`mailto:${contactLinks.email}`} className="block text-lg mb-2">
              {person.contact?.email}
            </a>
            <a href={`tel:${contactLinks.phone}`} className="block text-lg mb-2">
            ...
            {person.contact?.github && (
              <a href={person.contact.github} className="block text-lg mb-2">
                {person.contact.github}
              </a>
            )}
          </div>
      ...
      {/* Button placed outside the resumeRef */}
      <button onClick={handleCapture} className="bg-green-500 text-white px-4 py-2 rounded mt-4">
